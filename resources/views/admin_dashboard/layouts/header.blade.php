<?php
    ...
    $newComments = Comment::latest()->take(10)->get();
$posts_comments = [];
foreach ($newComments as $comment) {
    $posts_comments[] = ['comment' => $comment, 'post' => Post::find($comment->post_id)];
}
?>
                            ...
                            <li class="nav-item dropdown dropdown-large">
                                ...
                                <div class="dropdown-menu dropdown-menu-end">
                                    ...
                                    <div class="header-message-list">
                                        @for ($i = 0; $i < min(count($posts_comments), 10); $i++)
                                            @php
                                                $post = isset($posts_comments[$i]['post']) ? $posts_comments[$i]['post'] : $posts_comments[$i][0];
                                                $comment = isset($posts_comments[$i]['comment']) ? $posts_comments[$i]['comment'] : $post->comments()->orderBy('id', 'DESC')->first();
                                                $userImage = $comment->user->image ? asset('storage/' . $comment->user->image->path) : asset('storage/placeholders/user_placeholder.jpg');
                                            @endphp
                                            <a class="dropdown-item" href="{{ route('posts.show', $post) }}">
                                                <div class="d-flex align-items-center">
                                                    <div class="user-online">
                                                        <img class="img_admn--user img-avatar" width="50" height="50"
                                                             style="border-radius: 50%; margin: auto; background-size: cover; background-image: url('{{ $userImage }}')" alt="">
                                                    </div>
                                                    <div style="margin-left: 10px;" class="flex-grow-1">
                                                        <h6 class="msg-name">{{ $comment->user->name }}<span class="msg-time"> đã bình luận bài viết</span></h6>
                                                        <h6 class="msg-name">{{ Str::limit($post->title, 32) }}</h6>
                                                    </div>
                                                </div>
                                            </a>
                                        @endfor
